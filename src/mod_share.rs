...
pub struct BundlePos<T> {
  pub bundle: Arc<Vec<ItemOffset>>,
  pub pos: usize,
  pub len: usize,
  pub thing: T
}
...
pub fn update_top<I: IntoIterator<Item = usize>, T>(
  ...
  index_holder: &mut Vec<BundlePos<T>>,
  what: I,
) {
  for j in what {
    ...
    if tmp.pos < tmp.len {
      ...
      match top.get(&v.hash) {
        None => {
          top.insert(v.hash, vec![ItemOffsetPlusWhich{item_offset: v.clone(), which: j}]);
        }
        Some(vec) => {
          top.insert(v.hash, {
            let mut vec = vec.clone();
            vec.push(ItemOffsetPlusWhich{item_offset: v.clone(), which: j});
            vec
          });
        }
      }
